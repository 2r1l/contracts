...
const DAO = "0x7a2E1BC839ef2bE3Ae404e8f7397B70A1521987E"
...
async function main() {
    ...
    let txn = await dao.connect(proposer)["propose(address[],uint256[],string[],bytes[],string)"](
        [receiver.address], [treasury], [sig], ["0x"], "send funds")
    ...
    await hre.network.provider.request({
        ...
        params: [Date.now() + (await timelock.delay()).toNumber() * 2],
    });
